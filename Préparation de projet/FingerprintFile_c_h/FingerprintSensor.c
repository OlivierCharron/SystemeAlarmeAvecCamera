...
struct Adafruit_Fingerprint MonCapteur;

boolean verifyPassword(void) 
{
  uint8_t packet[] = {FINGERPRINT_VERIFYPASSWORD, 
                      (MonCapteur.thePassword >> 24 ),(MonCapteur.thePassword >> 16),
                      (MonCapteur.thePassword >> 8), (MonCapteur.thePassword};
  writePacket(MonCapteur.theAddress, FINGERPRINT_COMMANDPACKET, 7, packet);
...
uint8_t createModel(void) {
  ...
  writePacket(MonCapteur.theAddress, FINGERPRINT_COMMANDPACKET, sizeof(packet)+2, packet);
  uint8_t len = getReply(packet);
...
uint8_t fingerFastSearch(void) {
  ...
  MonCapteur.fingerID = packet[2];
  MonCapteur.fingerID <<= 8;
  MonCapteur.fingerID |= packet[3];
  
  MonCapteur.confidence = packet[4];
  MonCapteur.confidence <<= 8;
  MonCapteur.confidence |= packet[5];
  
  return packet[1];
}

uint8_t getTemplateCount(void) {
  MonCapteur.templateCount = 0xFFFF;
  ...
  MonCapteur.templateCount = packet[2];
  MonCapteur.templateCount <<= 8;
  MonCapteur.templateCount |= packet[3];
...
void writePacket(uint32_t addr, uint8_t packettype, 
